...
| Phase 2, Run 2 — regularization | + weight decay (1e-4) + conv-block dropout (0.2), isolated. Mirrors Phase 1's Run 2 exactly | `configs/phase2_reg.yaml` | `checkpoints/phase2_run2_regularization.pt` | `runs/phase2_run2_regularization/` | Done — see `results.md`. micro-F1 0.53, macro-F1 0.25 (test). Helps classes already partially learnable (`pia`/`sax`), but zeroes out `org`/`vio` which had non-zero scores in Run 1b. |
| Phase 2, Run 3 — SpecAugment | + time/frequency masking on input, isolated. Mirrors Phase 1's Run 3 exactly | `configs/phase2_specaug.yaml` | `checkpoints/phase2_run3_specaugment.pt` | `runs/phase2_run3_specaugment/` | Done — see `results.md`. micro-F1 0.50, macro-F1 0.26 (test, best macro of the three so far). Early-stopped epoch 23 (best epoch 16). Alone (no dropout/weight decay) does not close the overfitting gap the way it did in Phase 1. |
| Phase 2, Run 4 — combined | Run 2 + Run 3 together. Mirrors Phase 1's Run 4 | `configs/phase2_combined.yaml` | `checkpoints/phase2_run4_combined.pt` | `runs/phase2_run4_combined/` | Done — see `results.md`. micro-F1 0.50, macro-F1 0.24 (test) — **worst of the 4 Phase 2 runs**; combining compounds both techniques' costs on marginal classes (`gac` recall 0.56→0.10) rather than complementing. Closes out Phase 2's Phase A — no Run 5 follow-up planned (unlike Phase 1, this isn't an epoch-budget problem). |
